...
color_sequence = np.arange(mean_r[1:-2].size) / mean_r[1:-2].size

ax_contour.scatter(mean_r[1:-2], mean_z[1:-2], c=color_sequence, cmap=plt.cm.magma, marker='o', alpha=0.7, s=2)

# Mark the start and end points.
ax_contour.plot(mean_r[1], mean_z[1], 'go')
ax_contour.plot(mean_r[-2], mean_z[-2], 'r^')
# ax_contour.set_xlim(1400, 3100)
ax_contour.set_ylim(1000, 2000)
# ax_contour.colorbar()
# ax_contour.show()
...
ax_counts.set_ylabel("Nbre ")
# ax_counts.show()
f.savefig("brown_MDF_conv.png", dpi=400)
f.show()
