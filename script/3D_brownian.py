...
raw = np.loadtxt("particle_xyz.txt")
...
ax.plot3D(mean_x[1:-2], mean_y[1:-2], mean_z[1:-2], label="mvt brownien 3D", alpha=0.2)

ax.scatter(mean_x[1:-2], mean_y[1:-2], 600, c=color_sequence, cmap=plt.cm.magma, marker='o', alpha=0.2, s=2)
ax.plot3D(mean_x[1:-2], mean_y[1:-2], 600, label="Projection xy", alpha=0.2)
...
ax.set_zlabel('z / nm')
plt.legend()
plt.savefig("mvt_brownien_3D.png", dpi=400)
plt.show()